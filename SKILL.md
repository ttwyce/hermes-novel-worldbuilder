...
每个子代理独立生成一章（约3000字），不受截断限制。串行执行确保顺序。

```
① 主Agent调用delegate_task，目标="为《书名》写第X章，约3000字，路径=XXX"
② 子代理读取上下文（世界观/角色/前情）
③ 子代理用终端cat heredoc一次性写完章节（不用write_file）
④ 验证：python3 verify_chapter.py 第X章.md 3000
⑤ 通过后返回主Agent，启动下一章子代理
```
